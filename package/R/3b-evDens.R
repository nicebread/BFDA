...
#' @param boundary Critical BF boundaries for H0 and H1, e.g. \code{c(1/3, 6)}. If only one value is provided, the second boundary will be defined as the reciprocal.
#' @param xlim limits on xaxis
#' @param noSplit If TRUE, the par(mfcol=...) command is skipped. Set to TRUE if only one BFDA result should be plotted.
...
#' @details Either the argument \code{BFDA.H1} or the argument \code{BFDA.H0} or both can be used.

evDens <- function(BFDA.H1=NA, BFDA.H0=NA, n, boundary=c(1/6, 6), xlim=NA, noSplit=FALSE, cex=1.2, cex.axis=1, bw=FALSE) {
  
  N <- n	# rename, otherwise dplyr breaks ...
  
  # symmetric boundary if only one boundary is defined
  if(length(boundary) == 1) boundary <- sort(c(boundary, 1/boundary))
  
  # reduce data set to relevant sample size
  if(!all(is.na(BFDA.H0))){
    logBF.H0 <-  BFDA.H0$sim %>% filter(n==N) %>% .$logBF
  } else {
    logBF.H0 <- NULL
  }
  
  if(!all(is.na(BFDA.H1))){
    # reduce data set to relevant sample size
  ...
  if(!all(is.na(BFDA.H1))) suppressWarnings(evDens.plot(dens.H1, n=N, boundary=boundary, xlim=xlim, ylim=ylim, cex=cex, cex.axis=cex.axis, bw=bw, largeIsGreen=TRUE, main=expression(Under~H[1])))
  
  # ---------------------------------------------------------------------
  # Under H0
  
  if(!all(is.na(BFDA.H0))) suppressWarnings(evDens.plot(dens.H0, n=N, boundary=boundary, xlim=xlim, ylim=ylim, cex=cex, cex.axis=cex.axis, bw=bw, largeIsGreen=FALSE, main=expression(Under~H[0])))
