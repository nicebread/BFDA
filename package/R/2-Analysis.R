...
#' @param n.max At which maximum n should the simulations be analyzed?
#' @param boundary At which BF boundary should trajectories stop?
...
#' }
BPA.analysis <- function(BPA, n.max=NA, boundary=NA, verbose=TRUE, alpha=.05) {
	sim <- BPA$sim
	if (is.na(n.max)) n.max <- max(sim$n)
	if (is.na(boundary)) boundary <- max(sim$boundary)
		
	# reduce simulation to relevant data
	sim <- sim %>% filter(n <= n.max)
			
	logBoundary <- log(boundary)
		
	if (boundary > max(sim$boundary)) warning(paste0("Error: The selected boundary (", boundary, ") for analysis is larger than the smallest stopping boundary (", min(sim$boundary), ") in the simulation stage. Cannot produce a meaningful analysis."))
	...
	n.max.hit <- sim %>% group_by(id) %>% filter(n == n.max, max(abs(logBF)) <= logBoundary)

	# reduce to *first* break of a boundary
	boundary.hit <- sim %>% group_by(id) %>%
		filter(abs(logBF)>=logBoundary) %>%
...
#' @param digit Number of digits in display
...
with(x, {
	...
	if (n.max.traj.n > 0) {
		cat(paste0("\nOf ", round(n.max.traj.n/all.traj.n*100, digits), "% of studies terminating at n.max:\n",
			round(sum(n.max.hit.logBF > log(3))/all.traj.n*100), "% showed evidence for H1 (BF > 3)\n", 
			round(sum(n.max.hit.logBF < log(3) & n.max.hit.logBF > log(1/3))/all.traj.n*100), "% were inconclusive (3 > BF > 1/3)\n", 
			round(sum(n.max.hit.logBF < log(1/3))/all.traj.n*100), "% showed evidence for H0 (BF < 1/3)\n"
		))
	}
	
	# If some studies stopped at boundary: report ASN across all trials
	if (boundary.traj.n > 0) {	
		cat(paste0("\nAverage sample number (ASN) at stopping point (both boundary hits and n.max): n = ", ceiling(mean(endpoint.n))))
		cat("\n\nSample number quantiles (50/80/90/95%) at boundary hit:\n")
