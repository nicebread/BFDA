...
Let's go through the parameters of the simulation function:
  ...
  - `prior`: This argument specifies the prior distribution under the alternative hypothesis. It consists of a list with two elements: The first element is a character vector which specifies the type of the prior distribution. The second element is a list which contains the hyperparameters of the prior distribution. For example, the prior distribution for t-tests could be defined as `prior = list("Cauchy", prior.location = 0, prior.scale = 1))`. The choice of prior distributions and the default prior settings depend on the type of design (see next bullet point)
    ...
    - `type = "t.between"` or `type = "t.paired"`: There are 3 distribution types you can choose from (see list below). The default prior for t-tests is a zero-centered Cauchy distribution with a scale parameter of $\sqrt(2)/2$ which is equivalent to the default setting in the *BayesFactor* package. 
  ...
  - `design`: This argument specifies the experimental design which can either be a fixed-N design (`fixed.n`) or a sequential design (`sequential`). In the case of a sequential design, the arguments `n.min` and `boundary` are disregarded because they are irrelevant in this kind of design.
  ...
  - `seed`: Seed that ensures reproducibility with parallel processing. If the parameter is set to `NULL`, a new seed is chosen at each run.
...
The BFDA uses informed t-test functions by [Gronau, Ly, & Wagenmakers (2017)](https://arxiv.org/abs/1704.02479) to compute the between and paired t-test. By default, it uses a central Cauchy distribution with a scale parameter of sqrt(2)/2 as a prior on effect size under H1 as it is used in the *BayesFactor* package. The effect size metric is Cohen's d (standardized difference of means).
...
```{r correlation_walkthrough, warning=FALSE, fig.width=7, fig.height=4}
#devtools::install_github("nicebread/BFDA", subdir="package")
#library(BFDA)
...
The Bayesian AB test in the BFDA package is based on functions by Quentin Gronau (no paper/preprint available online yet). The test uses a normal prior on the difference of log odds as an analysis prior. The default is a standard normal distribution (mean = 0, variance = 1). In the BFDA, you can use an odds ratio, a log odds ratio, a relative risk, or an absolute risk as define a design prior on the population effect size (arguments `expected.ES` and `options.sample`).
