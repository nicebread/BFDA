...
> Schönbrodt, F. D. (2017). RSA: An R package for Bayes factor design analysis (version 0.2). Retrieved from https://github.com/nicebread/BFDA
...
sim.H1 <- BFDA.sim(expected.ES=0.5, type="t.between", n.min=20, n.max=300, alternative="directional", boundary=Inf, B=1000, verbose=TRUE, cores=4)

sim.H0 <- BFDA.sim(expected.ES=0, type="t.between", n.min=20, n.max=300, alternative="directional", boundary=Inf, B=1000, verbose=TRUE, cores=4)
```
...
s1 <- BFDA.sim(expected.ES=0.4, n.min=50, stepsize=5, n.max=300, type="t.paired", design="sequential", rscale=sqrt(2)/2, alternative="directional", B=1000, cores=4)
s0 <- BFDA.sim(expected.ES=0, n.min=50, stepsize=5, n.max=300, type="t.paired", design="sequential", rscale=sqrt(2)/2, alternative="directional", B=1000, cores=4)
...
The correlation BF implemented in the BFDA package uses a beta prior distribution, as described in:
...
> "We prefer a similar but more elegant solution, where we first rescale the correlation to lie between 0 and 1, and then assign it a beta distribution. Hence, ρ′ ∼ beta(α, α), and a measure of the spread of this distribution is γ = 1/α. We then transform the beta distribution back to the (−1, 1) scale and calculate the Bayes factors as a function of γ . When γ = 1, this corresponds to a uniform prior on the correlation coefficient, as per our default analysis. When γ = 0, which happens when α grows very large, H1 becomes indistinguishable from H0 and consequently the Bayes factor is 1. Values of γ in between 0 and 1 define an continuous range of different alternative hypotheses that represent different beliefs about the extent to which large values for the correlation are plausible." (p. 419)

The alpha parameter which can be passed as a parameter in the BFDA.sim function corresponds to the "beta* prior width" in JASP. Here are some plots for different settings of alpha for the two-sided case:

```{r corr_alpha_prior_2_plots, cache=TRUE, results='hide'}
rho <- seq(-1, 1, by=.01)

par(mfrow=c(2, 3))
plot(rho, BFDA:::priorRho(rho, alpha=1),    main="alpha = 1",    ylim=c(0, 2.4), type="l", xlab="rho", ylab="Plausibility")
plot(rho, BFDA:::priorRho(rho, alpha=1.5),  main="alpha = 1.5",  ylim=c(0, 2.4), type="l", xlab="rho", ylab="Plausibility")
plot(rho, BFDA:::priorRho(rho, alpha=2),    main="alpha = 2",    ylim=c(0, 2.4), type="l", xlab="rho", ylab="Plausibility")
plot(rho, BFDA:::priorRho(rho, alpha=3),    main="alpha = 3",    ylim=c(0, 2.4), type="l", xlab="rho", ylab="Plausibility")
plot(rho, BFDA:::priorRho(rho, alpha=0.75), main="alpha = 0.75", ylim=c(0, 2.4), type="l", xlab="rho", ylab="Plausibility")
plot(rho, BFDA:::priorRho(rho, alpha=0.5),  main="alpha = 0.5",  ylim=c(0, 2.4), type="l", xlab="rho", ylab="Plausibility")
```

Here are some plots for different settings of alpha for the one-sided case:

```{r corr_alpha_prior_1_plots, cache=TRUE, results='hide'}
rho <- seq(0, 1, by=.01)

par(mfrow=c(2, 3))
plot(rho, BFDA:::priorRhoPlus(rho, alpha=1),    main="alpha = 1",    ylim=c(0, 4.2), type="l", xlab="rho", ylab="Plausibility")
plot(rho, BFDA:::priorRhoPlus(rho, alpha=1.5),  main="alpha = 1.5",  ylim=c(0, 4.2), type="l", xlab="rho", ylab="Plausibility")
plot(rho, BFDA:::priorRhoPlus(rho, alpha=2),    main="alpha = 2",    ylim=c(0, 4.2), type="l", xlab="rho", ylab="Plausibility")
plot(rho, BFDA:::priorRhoPlus(rho, alpha=3),    main="alpha = 3",    ylim=c(0, 4.2), type="l", xlab="rho", ylab="Plausibility")
plot(rho, BFDA:::priorRhoPlus(rho, alpha=0.75), main="alpha = 0.75", ylim=c(0, 4.2), type="l", xlab="rho", ylab="Plausibility")
plot(rho, BFDA:::priorRhoPlus(rho, alpha=0.5),  main="alpha = 0.5",  ylim=c(0, 4.2), type="l", xlab="rho", ylab="Plausibility")
```


Here's a complete walkthrough for a one-sided (directional) correlation BF design analysis, which places prior weight on smaller effect sizes (alpha = 2):

```{r correlation_walkthrough}
#devtools::install_github("nicebread/BFDA", subdir="package")
library(BFDA)

# do a sequential design analysis
# scale the beta* prior with the alpha parameter
s1 <- BFDA.sim(expected.ES=0.21, n.min=50, stepsize=10, n.max=300, B=100, type="correlation", design="sequential", alpha=2, alternative="directional", cores=4)
s0 <- BFDA.sim(expected.ES=0, n.min=50, stepsize=10, n.max=300, B=100, type="correlation", design="sequential", alpha=2, alternative="directional", cores=4)
